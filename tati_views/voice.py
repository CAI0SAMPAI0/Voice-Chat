...
inject_css('voice')
inject_css('sidebar')
import streamlit.web.server.websocket_headers as _wsh
_iframe_h = 900

# pegando altura pelo query param
try:
...
html,body{overflow:hidden!important;}
    ...
    st.session_state["_sidebar_was_open"] = True
    st.markdown("""<style>
...
// ── Footer fixo ──
function fixFooter(){{
    var footer = document.querySelector('.mic-footer');
    ...
    footer.style.cssText = [
        ...
        'background:#060a10','border-top:1px solid #1a2535',
        'box-sizing:border-box','display:flex','flex-direction:column',
        'align-items:center','gap:8px','padding:10px 16px 14px',
        'height:auto','overflow:visible'
    ].map(function(r){{ return r+'!important'; }}).join(';') + ';';

    // Força filhos visíveis
    var children = footer.querySelectorAll('*');
    children.forEach(function(el){{
        el.style.setProperty('display', el.tagName === 'INPUT' ? 'block' : 
                             el.classList.contains('audio-controls') ? 'flex' : 
                             el.classList.contains('mic-btn') ? 'flex' : 'block', 'important');
        el.style.setProperty('visibility', 'visible', 'important');
        el.style.setProperty('opacity', '1', 'important');
        el.style.setProperty('height', 'auto', 'important');
        el.style.setProperty('overflow', 'visible', 'important');
    }});
    var micBtn = footer.querySelector('.mic-btn');
    if(micBtn){{
        micBtn.style.setProperty('width',  '72px', 'important');
        micBtn.style.setProperty('height', '72px', 'important');
        micBtn.style.setProperty('min-width',  '72px', 'important');
        micBtn.style.setProperty('min-height', '72px', 'important');
        micBtn.style.setProperty('font-size', '28px', 'important');
        micBtn.style.setProperty('border-radius', '50%', 'important');
        micBtn.style.setProperty('flex-shrink', '0', 'important');
    }}
    [100, 350, 700].forEach(function(d){{
        setTimeout(function(){{
            var fh = footer.getBoundingClientRect().height || 140;
            var extra = fh + 16;
            hw.style.setProperty('padding-bottom', extra + 'px', 'important');
            hw.style.setProperty('overflow-y', 'auto', 'important');
...
setTimeout(function(){{
    ...
    console.log('footer height:', footer ? footer.getBoundingClientRect().height : 'N/A');
    console.log('footer position computed:', footer ? window.getComputedStyle(footer).position : 'N/A');
    console.log('footer bottom computed:', footer ? window.getComputedStyle(footer).bottom : 'N/A');
    console.log('footer display computed:', footer ? window.getComputedStyle(footer).display : 'N/A');
    console.log('hw paddingBottom:', hw ? hw.style.paddingBottom : 'N/A');
...
(function resizeIframe(){{
    // Salva viewport height no URL para o Python ler no próximo rerun
    try{{
        var vh = window.parent.innerHeight;
        if(window.parent.visualViewport) vh = Math.round(window.parent.visualViewport.height);
        var url = new URL(window.parent.location.href);
        if(url.searchParams.get('vh') !== String(vh)){{
            url.searchParams.set('vh', vh);
            window.parent.history.replaceState(null, '', url.toString());
        }}
    }}catch(e){{}}

    // Ajusta altura do iframe para ocupar viewport inteiro
    try{{
    ...
    // Aplica footer fixo após o iframe ter a altura certa
    fixFooter();

    // Registra listeners para recalcular quando tamanho mudar
    try{{
...
// ResizeObserver no body do iframe — dispara quando sidebar abre/fecha
try{{
    var _ro = new ResizeObserver(function(){{ fixFooter(); }});
...
def _get_api_key() -> str:
    """Busca ANTHROPIC_API_KEY simples ou numerada _1.._4."""
    for name in ["ANTHROPIC_API_KEY"] + [f"ANTHROPIC_API_KEY_{i}" for i in range(1, 5)]:
        v = os.getenv(name, "")
        if v:
            return v
    try:
