"""
tati_views/history.py — Tela de histórico de conversas.

Mudanças vs versão anterior:
─────────────────────────────
1. REMOVIDOS os dois blocos st.markdown(<style>...</style>) inline
   (93 linhas no total). Todo o CSS foi consolidado em history.css.
   O Python não deve conter estilos — apenas classes semânticas.

2. REMOVIDO inject_css('sidebar') — já injetado pelo inject_global_css()
   em app.py. Era duplicado.

3. Adicionado wrapper .hist-toggle-row no toggle "👁 Ver mensagens"
   para substituir o hack no seletor stTooltipHoverTarget que estava
   em history.css (seletor interno do Streamlit, frágil).

4. Removido style="padding-top:8px;" inline do .hist-del-row —
   esse espaçamento agora vive em history.css (.hist-del-row).

5. _render_preview(): @import movido para o início do <style> —
   em HTML, @import deve ser a PRIMEIRA regra dentro de <style>,
   caso contrário é ignorado pelo browser.
"""

import json
from datetime import datetime

import streamlit as st
import streamlit.components.v1 as components

from database import (
    list_conversations,
    load_conversation,
    delete_conversation,
)
from ui_helpers import t, PROF_NAME
from guards.page_guard import page_guard, scroll_restore
from asset_loader import inject_css


@page_guard
def show_history() -> None:
    user     = st.session_state.user
    username = user["username"]
    profile  = user.get("profile", {})
    lang     = profile.get("language", "pt-BR")

    scroll_restore()
    inject_css("history")   # sidebar NÃO vai aqui — já vem do inject_global_css()

    convs = list_conversations(username)

    # ── Header ───────────────────────────────────────────────────────────────
    count_badge = (
        f"<span class='hist-count'>{len(convs)} conversa{'s' if len(convs) != 1 else ''}</span>"
        if convs else ""
    )
    st.markdown(
        f"<div class='hist-header'>"
        f"<h2>📄 {t('history', lang)}</h2>"
        f"{count_badge}"
        f"</div>",
        unsafe_allow_html=True,
    )

    if not convs:
        st.markdown(
            f"<div class='hist-empty'>💬 {t('no_history', lang)}</div>",
            unsafe_allow_html=True,
        )
        return

    # ── Lista de conversas ────────────────────────────────────────────────────
    for conv in convs:
        cid       = conv["id"]
        title     = conv.get("title") or conv.get("first_message", "") or f"Conversa {cid[:8]}"
        date_raw  = conv.get("updated_at", "") or conv.get("created_at", "") or conv.get("date", "")
        msg_count = conv.get("msg_count", 0)

        # Formata data
        date_fmt = date_raw[:16] if date_raw else "---"
        try:
            _d = datetime.fromisoformat(date_raw[:19])
            date_fmt = _d.strftime("%d/%m/%Y %H:%M")
        except Exception:
            pass

        col_open, col_del = st.columns([12, 1])

        with col_open:
            # Card de conversa
            st.markdown(f"""
<div class="hist-card">
    <div class="hist-card-icon">🎙️</div>
    <div class="hist-card-body">
        <div class="hist-card-title">{title[:70]}{'…' if len(title) > 70 else ''}</div>
        <div class="hist-card-meta">
            <div class="hist-meta-item">📅 {date_fmt}</div>
            <div class="hist-meta-dot"></div>
            <div class="hist-meta-item">💬 {msg_count} msg{'s' if msg_count != 1 else ''}</div>
        </div>
    </div>
</div>""", unsafe_allow_html=True)

            # Botão "Abrir conversa"
            st.markdown('<div class="hist-open-row">', unsafe_allow_html=True)
            if st.button("▶ Abrir conversa", key=f"conv_{cid}", help=title):
                st.session_state.conv_id          = cid
                st.session_state["_vm_history"]   = []
                st.session_state["_vm_reply"]     = ""
                st.session_state["_vm_user_said"] = ""
                st.session_state.page             = "voice"
                st.rerun()
            st.markdown('</div>', unsafe_allow_html=True)

            # Toggle de preview
            # Wrapper .hist-toggle-row substitui o hack de margem via
            # div[data-testid="stTooltipHoverTarget"] que estava em history.css
            st.markdown('<div class="hist-toggle-row">', unsafe_allow_html=True)
            if st.toggle("👁 Ver mensagens", key=f"preview_{cid}"):
                msgs = load_conversation(username, cid)
                if msgs:
                    _render_preview(msgs, cid)
                else:
                    st.markdown(
                        "<div style='color:var(--text-muted);font-size:.8rem;"
                        "padding:8px 16px;'>Sem mensagens.</div>",
                        unsafe_allow_html=True,
                    )
            st.markdown('</div>', unsafe_allow_html=True)

        with col_del:
            # Espaçamento via .hist-del-row no CSS — sem style inline
            st.markdown('<div class="hist-del-row">', unsafe_allow_html=True)
            if st.button("🗑", key=f"del_{cid}", help=t("del_conv", lang)):
                delete_conversation(username, cid)
                if st.session_state.conv_id == cid:
                    st.session_state.conv_id = None
                st.rerun()
            st.markdown('</div>', unsafe_allow_html=True)

    st.markdown("<div style='height:14px'></div>", unsafe_allow_html=True)


def _render_preview(msgs: list, conv_id: str) -> None:
    """
    Renderiza bolhas da conversa em um iframe isolado.
    Mensagens do bot com tts_b64 ganham botão ▶ Ouvir.

    CORRIGIDO: @import movido para o início do <style>.
    Em HTML, @import deve ser a PRIMEIRA regra dentro de <style>.
    Se estiver depois de qualquer outra declaração, o browser ignora.
    """
    bubbles_js   = json.dumps([
        {
            "role":    m["role"],
            "content": (m.get("content") or "")[:300],
            "tts_b64": m.get("tts_b64", ""),
        }
        for m in msgs if m.get("content")
    ])
    prof_name_js = json.dumps(PROF_NAME)

    components.html(f"""<!DOCTYPE html><html><head>
<meta charset="UTF-8">
<style>
@import url('https://fonts.googleapis.com/css2?family=Sora:wght@400;600&display=swap');
/* Reset e base */
*, *::before, *::after {{ box-sizing: border-box; margin: 0; padding: 0; }}
html, body {{
    background: #070c15;
    font-family: 'Sora', sans-serif;
    padding: 12px 16px;
    color: #e6edf3;
}}
/* Container de bolhas */
.wrap {{
    display: flex;
    flex-direction: column;
    gap: 6px;
}}
/* Labels de nome */
.label {{
    font-size: .6rem;
    color: #4a5a6a;
    margin: 4px 3px 1px;
}}
.label.right {{ text-align: right; }}
/* Bolhas */
.bubble {{
    max-width: 82%;
    padding: 8px 12px;
    border-radius: 14px;
    font-size: .8rem;
    line-height: 1.5;
    word-break: break-word;
}}
.bubble.user {{
    align-self: flex-end;
    background: #2d6a4f;
    color: #fff;
    border-bottom-right-radius: 4px;
    margin-left: auto;
}}
.bubble.bot {{
    align-self: flex-start;
    background: #1a1f2e;
    color: #e6edf3;
    border: 1px solid #252d3d;
    border-bottom-left-radius: 4px;
}}
/* Botão play */
.play-btn {{
    align-self: flex-start;
    background: transparent;
    border: 1px solid #1a2535;
    color: #3a6a8a;
    font-size: .7rem;
    padding: 3px 10px;
    border-radius: 6px;
    cursor: pointer;
    font-family: inherit;
    transition: all .15s;
    margin-bottom: 2px;
}}
.play-btn:hover  {{ color: #f0a500; border-color: rgba(240,165,0,.4); background: rgba(240,165,0,.06); }}
.play-btn.playing {{ color: #e05c2a; border-color: rgba(224,92,42,.5); }}
</style>
</head><body>
<div class="wrap" id="wrap"></div>
<script>
(function() {{
    var BUBBLES   = {bubbles_js};
    var PROF_NAME = {prof_name_js};
    var wrap      = document.getElementById('wrap');
    var currentAudio = null;

    function playAudio(b64, btn) {{
        if (currentAudio && !currentAudio.paused) {{
            currentAudio.pause();
            currentAudio = null;
            document.querySelectorAll('.play-btn').forEach(function(b) {{
                b.textContent = '\u25b6 Ouvir';
                b.classList.remove('playing');
            }});
        }}
        if (!b64) return;
        var audio = new Audio('data:audio/mp3;base64,' + b64);
        audio.volume = 1.0;
        currentAudio = audio;
        btn.textContent = '\u23f9 Parar';
        btn.classList.add('playing');
        audio.onended = function() {{
            btn.textContent = '\u25b6 Ouvir';
            btn.classList.remove('playing');
            currentAudio = null;
        }};
        audio.onerror = function() {{
            btn.textContent = '\u25b6 Ouvir';
            btn.classList.remove('playing');
            currentAudio = null;
        }};
        audio.play().catch(function() {{}});
    }}

    BUBBLES.forEach(function(msg) {{
        var role = msg.role === 'user' ? 'user' : 'bot';

        var lbl = document.createElement('div');
        lbl.className   = 'label' + (role === 'user' ? ' right' : '');
        lbl.textContent = role === 'user' ? 'Você' : PROF_NAME;

        var bub = document.createElement('div');
        bub.className   = 'bubble ' + role;
        bub.textContent = msg.content + (msg.content.length === 300 ? '\u2026' : '');

        wrap.appendChild(lbl);
        wrap.appendChild(bub);

        if (role === 'bot' && msg.tts_b64) {{
            var btn = document.createElement('button');
            btn.className   = 'play-btn';
            btn.textContent = '\u25b6 Ouvir';
            btn.addEventListener('click', function() {{
                var isPlaying = currentAudio && !currentAudio.paused;
                if (isPlaying && btn.classList.contains('playing')) {{
                    currentAudio.pause();
                    currentAudio = null;
                    btn.textContent = '\u25b6 Ouvir';
                    btn.classList.remove('playing');
                }} else {{
                    playAudio(msg.tts_b64, btn);
                }}
            }});
            wrap.appendChild(btn);
        }}
    }});
}})();
</script>
</body></html>""", height=min(400, len(msgs) * 70 + 40), scrolling=True)